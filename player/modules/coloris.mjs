...
let container; let picker; let colorArea; let colorMarker; let colorPreview; let colorValue; let clearButton; let closeButton;
let hueSlider; let hueMarker; let alphaSlider; let alphaMarker; let currentEl; let currentFormat; let oldColor; let keyboardNav;
let colorAreaDims = {};
// Default settings
const settings = {
  el: '[data-coloris]',
...
function attachVirtualInstance(element) {
  if (hasInstance) {
    ...
    const unsupportedOptions = ['el', 'wrap', 'rtl', 'inline', 'defaultColor', 'a11y']; const _loop = function _loop(
        selector) {
      ...
      const _ret = _loop(selector); if (_ret === 'break') break;
    }
  }
}
/**
...
function updatePickerPosition() {
  ...
  let parentStyle; let parentMarginTop; let parentBorderTop;
  let offset = {x: 0, y: 0};
  if (parent) {
...
function moveMarker(event) {
  ...
  const x = pointer.pageX - colorAreaDims.x;
...
function getFocusableElements() {
  ...
  const focusables = controls.filter(function(node) {
